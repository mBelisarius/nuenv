...
namespace internal {
    ...
    struct ConstsG10K21
    {
        static constexpr size_t n_g = 10;
        static constexpr size_t n_k = 21;

        static const VectorX_s<Scalar, n_g> w_g;
        static const VectorX_s<Scalar, n_g> x_g;

        static const VectorX_s<Scalar, n_k> w_k;
        static const VectorX_s<Scalar, n_k> x_k;
    };

    template<typename Scalar>
    const VectorX_s<Scalar,
                    ConstsG10K21<Scalar>::n_g> ConstsG10K21<Scalar>::w_g =
    {
    ...
    template<typename Scalar>
    const VectorX_s<Scalar,
                    ConstsG10K21<Scalar>::n_g> ConstsG10K21<Scalar>::x_g =
    {
    ...
    template<typename Scalar>
    const VectorX_s<Scalar,
                    ConstsG10K21<Scalar>::n_k> ConstsG10K21<Scalar>::w_k =
    {
    ...
    template<typename Scalar>
    const VectorX_s<Scalar,
                    ConstsG10K21<Scalar>::n_k> ConstsG10K21<Scalar>::x_k =
    {
                   ...
                   Scalar tol = 6e-6)
{
    ...
    // TODO: Vectorize
    for (size_t i = 0; i < ConstsG10K21<Scalar>::n_g; i++)
    {
        aux = ((b - a) * ConstsG10K21<Scalar>::x_g[i] + (b + a)) / 2.0;
        integral_g += ConstsG10K21<Scalar>::w_g[i] * func(aux);
    }
    ...
    // TODO: Vectorize
    for (size_t i = 0; i < ConstsG10K21<Scalar>::n_k; i++)
    {
        aux = ((b - a) * ConstsG10K21<Scalar>::x_k[i] + (b + a)) / 2.0;
        integral_k += ConstsG10K21<Scalar>::w_k[i] * func(aux);
    }
    ...
    {
        size_t n = ceil(1.0 + log2(error / tol));
